...
import { AirflowDetailPage } from "./AirflowDetailPage";
...
const CATALOG_ITEMS = [
  { id: "chroma",     title: "Chroma DB",   desc: "open-source embedding database for building AI applications with vector similarity search",  logo: logoChroma },
  { id: "codeserver", title: "Code server", desc: "VS code in the browser with full development environment and extensions support",             logo: logoCodeserver },
  { id: "jupyterlab", title: "JupyterLab",  desc: "Interactive development environment for jupyter notebooks with Python kernel",                logo: logoJupyterlab },
  { id: "langflow",   title: "Langflow",    desc: "Visual framework for building multi-agent and RAG applications with drag-and-drop interface", logo: logoLangflow },
  { id: "milvus",     title: "Milvus",      desc: "Cloud-native vector database built for scalable similarity search and AI applications",       logo: logoMilvus },
  { id: "qdrant",     title: "Qdrant",      desc: "Cloud-native vector database built for scalable similarity search and AI applications",       logo: logoQdrant },
  { id: "airflow",    title: "Airflow (Managed)", desc: "Data workflow orchestration dashboard",                                                      logo: logoAirflow },
];
...
export function CatalogPage({ onNavigate }: CatalogPageProps) {
  ...
  if (detailApp === "airflow") {
    return (
      <AirflowDetailPage
        onBack={() => setDetailApp(null)}
        onNavigate={(key) => { setDetailApp(null); handleNavSelect(key); }}
      />
    );
  }
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <main style={{ flex: 1, overflow: "auto", padding: 24 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 32 }}>
            <h1 style={{ fontSize: 24, fontWeight: 600, lineHeight: "32px", color: colors.text.primary, fontFamily: "'Pretendard', sans-serif" }}>
              Catalog
            </h1>
            <p style={{ fontSize: 14, fontWeight: 400, lineHeight: "16px", color: colors.text.secondary, fontFamily: "'Pretendard', sans-serif" }}>
              Explore available apps and models
            </p>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "32px 8px" }}>
            {CATALOG_ITEMS.map((item) => (
              <GridCard
                key={item.id}
                title={item.title}
                desc={item.desc}
                onClick={item.id === "airflow" ? () => setDetailApp("airflow") : undefined}
              >
                <img
                  src={item.logo}
                  alt={item.title}
                  style={{ maxWidth: "65%", maxHeight: "41%", objectFit: "contain", display: "block" }}
                />
              </GridCard>
            ))}
          </div>
